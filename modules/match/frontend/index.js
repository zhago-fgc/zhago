...
function updateCharacterControls(i) {
  ...
  rows.forEach((row) => row.querySelector('.remove-character').classList.toggle('hidden', rows.length <= min))
}
...
function addCharacterRow(i, value = '') {
  ...
  select.innerHTML = '<option value="">— character —</option>' +
    gameData.characters.map((c) => '<option value="' + c.name + '">' + c.name + '</option>').join('')
...
function sideValues(i) {
  return {
    players: [...playerContainers[i].querySelectorAll('input')].map((el) => el.value).filter(Boolean),
    characters: [...charContainers[i].querySelectorAll('select')].map((el) => el.value).filter(Boolean),
  }
}
