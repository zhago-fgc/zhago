  ...
  <div class="flex h-screen">
    <aside class="w-52 shrink-0 bg-zinc-950 flex flex-col border-r border-zinc-800 select-none">
      ...
      <nav class="flex-1 overflow-y-auto p-3 flex flex-col gap-0.5">
        <div class="mb-1 px-3 text-[10px] text-zinc-600 uppercase tracking-widest font-medium">Modules</div>
        <RouterLink
          v-for="m in withCockpit"
          :key="m.name"
          :to="`/m/${m.name}`"
          class="px-3 py-1.5 rounded text-sm transition-colors flex items-center justify-between gap-2 text-zinc-400 hover:text-white hover:bg-zinc-800/60"
          active-class="!bg-zinc-800 !text-white"
        >
          <span class="capitalize">{{ m.name }}</span>
          <span class="text-[10px] text-zinc-600 uppercase">{{ m.type }}</span>
        </RouterLink>
        <p v-if="!withCockpit.length" class="px-3 text-sm text-zinc-600">No modules with a UI yet.</p>
      </nav>

      <div class="px-4 py-3 border-t border-zinc-800 text-[10px] text-zinc-600">zhago-bun experiment</div>
    </aside>
